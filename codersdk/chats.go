...
func (c *Client) RegenerateChatTitle(ctx context.Context, chatID uuid.UUID) (Chat, error) {
	res, err := c.Request(ctx, http.MethodPost, fmt.Sprintf("/api/v2/chats/%s/title/regenerate", chatID), nil)
	if err != nil {
		return Chat{}, err
	}
	defer res.Body.Close()
