...
func (c *ExperimentalClient) ClearChat(ctx context.Context, chatID uuid.UUID) (Chat, error) {
	res, err := c.Request(ctx, http.MethodPost, fmt.Sprintf("/api/experimental/chats/%s/clear", chatID), nil)
	if err != nil {
		return Chat{}, err
	}
	defer res.Body.Close()
