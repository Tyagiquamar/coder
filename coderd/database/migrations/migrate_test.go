...
func TestMigration000586WorkspaceAgentSessionCountsGuard(t *testing.T) {
	t.Parallel()

	const priorMigrationVersion = 585
	...
	ctx := testutil.Context(t, testutil.WaitSuperLong)
	migrationSQL, err := os.ReadFile("000586_workspace_agent_session_counts.up.sql")
	require.NoError(t, err)

	// The guard measures the span of session activity the rollup never consumed,
	// so a stale watermark only aborts when rows actually piled up behind it.
	tests := []struct {
		name              string
		statsAgeHours     []int
		sessionCount      int
		...
		{name: "empty database"},
		{name: "no watermark and brief activity", statsAgeHours: []int{0}, sessionCount: 2, wantCounts: `{"vscode": 2}`},
		{name: "no watermark and idle stats", statsAgeHours: []int{0}, wantCounts: `{}`},
		{name: "no watermark and a long activity backlog", statsAgeHours: []int{48, 0}, sessionCount: 2, wantError: true},
		{name: "expired activity only", statsAgeHours: []int{181 * 24}, sessionCount: 2, wantCounts: `{}`},
		...
		// behind it. The clock-based guard rejected this.
		{name: "weekend shutoff", statsAgeHours: []int{63}, sessionCount: 2, watermarkAgeHours: []int{64}, wantCounts: `{"vscode": 2}`},
		{name: "idle since the last rollup", statsAgeHours: []int{0}, watermarkAgeHours: []int{48}, wantCounts: `{}`},
		{name: "stalled rollup", statsAgeHours: []int{47, 0}, sessionCount: 2, watermarkAgeHours: []int{48}, wantError: true},
		{name: "activity already rolled up", statsAgeHours: []int{50}, sessionCount: 2, watermarkAgeHours: []int{25}, wantCounts: `{}`},
		{name: "fresh watermark", statsAgeHours: []int{0}, sessionCount: 2, watermarkAgeHours: []int{23}, wantCounts: `{"vscode": 2}`},
		{name: "latest watermark wins", statsAgeHours: []int{0}, sessionCount: 2, watermarkAgeHours: []int{25, 23}, wantCounts: `{"vscode": 2}`},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantError {
				require.ErrorContains(t, err, "that template usage stats never rolled up")
				var pqErr *pq.Error
				require.True(t, errors.As(err, &pqErr))
				require.Contains(t, pqErr.Hint, "failed to rollup data")
				...
				err = sqlDB.QueryRowContext(ctx, `
					SELECT EXISTS (
						...
						WHERE table_schema = 'public'
							...
							AND column_name = 'session_counts'
					)
				`).Scan(&columnExists)
				require.NoError(t, err)
			...
			if tt.wantCounts != "" {
				...
				err = tx.QueryRowContext(ctx, `SELECT session_counts FROM workspace_agent_stats`).Scan(&sessionCounts)
				require.NoError(t, err)
				require.JSONEq(t, tt.wantCounts, string(sessionCounts))
			}
		})
	}
	...
	t.Run("down restores known apps only", func(t *testing.T) {
		...
		downSQL, err := os.ReadFile("000586_workspace_agent_session_counts.down.sql")
		require.NoError(t, err)
