...
BEGIN
	...
	-- backlog, and neither should be blocked from upgrading.
	...
	IF backlog_end - backlog_start > interval '24 hours' THEN
		RAISE EXCEPTION 'migration 000586 found % of workspace agent stats that template usage stats never rolled up', justify_hours(backlog_end - backlog_start)
			USING DETAIL = format(
				'This migration converts every workspace_agent_stats row the rollup has not consumed, %s through %s, and holds ACCESS EXCLUSIVE on the table until it finishes. Template insights read those rows, so the backlog cannot be skipped. No schema change has been applied.',
				backlog_start, backlog_end
			),
			HINT = 'The rollup runs every 5 minutes, so start the previous Coder version and let it work through the backlog, then retry the upgrade. If the backlog does not shrink, the rollup is failing or blocked: check coderd logs for "failed to rollup data", and check pg_locks for a session holding the rollup advisory lock.';
	END IF;
END
$$;
...
-- Convert the window DeleteOldWorkspaceAgentStats retains, which is what the
-- rollups and operational statistics still read. Anything older has already
-- been rolled up, except where nothing has ever rolled up, and the guard above
-- rejects that case well before it reaches this far back.
