...
func (s *MethodTestSuite) TestUser() {
	...
	s.Run("AcquireUserSoftDeleteGuardLock", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		u := testutil.Fake(s.T(), faker, database.User{})
		dbm.EXPECT().GetUserByID(gomock.Any(), u.ID).Return(u, nil).AnyTimes()
		dbm.EXPECT().AcquireUserSoftDeleteGuardLock(gomock.Any(), u.ID).Return(nil).AnyTimes()
		check.Args(u.ID).Asserts(u, policy.ActionRead)
	}))
...
func (s *MethodTestSuite) TestUserMemories() {
	...
	s.Run("DeleteUserMemoriesByUserIDAndPathPrefix", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		...
		deleted := testutil.Fake(s.T(), faker, database.DeleteUserMemoriesByUserIDAndPathPrefixRow{UserID: user.ID})
		dbm.EXPECT().DeleteUserMemoriesByUserIDAndPathPrefix(gomock.Any(), arg).Return([]database.DeleteUserMemoriesByUserIDAndPathPrefixRow{deleted}, nil).AnyTimes()
		check.Args(arg).
			Asserts(rbac.ResourceUserMemory.WithOwner(user.ID.String()), policy.ActionDelete).
			Returns([]database.DeleteUserMemoriesByUserIDAndPathPrefixRow{deleted})
	}))
}
...
func (s *MethodTestSuite) TestChatMemories() {
	...
	s.Run("DeleteChatMemoriesByRootChatIDAndPathPrefix", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		...
		deleted := testutil.Fake(s.T(), faker, database.DeleteChatMemoriesByRootChatIDAndPathPrefixRow{RootChatID: chat.ID})
		dbm.EXPECT().GetChatByID(gomock.Any(), chat.ID).Return(chat, nil).AnyTimes()
		dbm.EXPECT().DeleteChatMemoriesByRootChatIDAndPathPrefix(gomock.Any(), arg).Return([]database.DeleteChatMemoriesByRootChatIDAndPathPrefixRow{deleted}, nil).AnyTimes()
		check.Args(arg).Asserts(chat, policy.ActionUpdate).Returns([]database.DeleteChatMemoriesByRootChatIDAndPathPrefixRow{deleted})
	}))
}
