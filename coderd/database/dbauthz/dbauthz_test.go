...
func (s *MethodTestSuite) TestChats() {
	...
	s.Run("UpdateChatQueuedMessageContent", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		chat := testutil.Fake(s.T(), faker, database.Chat{})
		queued := testutil.Fake(s.T(), faker, database.ChatQueuedMessage{ChatID: chat.ID})
		arg := database.UpdateChatQueuedMessageContentParams{
			ChatID:  chat.ID,
			ID:      queued.ID,
			Content: queued.Content,
		}
		dbm.EXPECT().GetChatByID(gomock.Any(), chat.ID).Return(chat, nil).AnyTimes()
		dbm.EXPECT().UpdateChatQueuedMessageContent(gomock.Any(), arg).Return(queued, nil).AnyTimes()
		check.Args(arg).Asserts(chat, policy.ActionUpdate).Returns(queued)
	}))
