...
// WorkspaceAgentStat inserts a workspace agent stat row. At most one map may be
// given, and it seeds the session_counts column.
func WorkspaceAgentStat(t testing.TB, db database.Store, orig database.WorkspaceAgentStat, sessionCounts ...map[string]int64) database.WorkspaceAgentStat {
	require.LessOrEqual(t, len(sessionCounts), 1, "at most one session counts map")
